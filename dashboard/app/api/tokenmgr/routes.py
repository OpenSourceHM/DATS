...
from app.api.tokenmgr.mgr import revoke_token, is_token_revoked, add_token_to_database
from app.api.tokenmgr import blueprint
...
"""
Making a request with a refresh token looks just like making a request with an access token. 
Here is an example:
    ...
    post:
      ...
      parameters:
        - in: header
          name: Authorization
          required: true
          description: valid refresh token as Authorization:"Bearer $REFRESH_TOKEN"
      responses:
        200:
          content:
            application/json:
...
def register_views():
    apispec.spec.path(view=auth, app=app)
